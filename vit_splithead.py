...
from layers_methods import TransformerEncoder

class ViT_SplitHead(nn.Module):
    def __init__(self, in_c:int=3, num_classes:int=10, img_size:int=32, patch:int=8, dropout:float=0., num_layers:int=7, hidden:int=384, mlp_hidden:int=384*4, head:int=8, is_cls_token:bool=True, head_mix_method:int=0):
        super().__init__()
        # hidden=384
        ...
        enc_list = [TransformerEncoder(hidden,mlp_hidden=mlp_hidden, dropout=dropout, head=head, head_mix_method=head_mix_method) for _ in range(num_layers)]
